...
export const selectIssue = (projectId: Project['project']['id'], issueId: Issue['id']) => (state: AppState) =>
  state.projects[projectId]?.issueBoards?.flatMap((board: IssueBoard) => board.items)?.find((issue) => issue.id === issueId);

export const selectIssueBoards = (projectId: Project['project']['id']) => (state: AppState) => state.projects[projectId]?.issueBoards;

export const selectProject = (projectId: Project['project']['id']) => (state: AppState) => state.projects[projectId];

export const selectProjectTitle = (projectId: Project['project']['id']) => (state: AppState) => state.projects[projectId]?.project.title;
