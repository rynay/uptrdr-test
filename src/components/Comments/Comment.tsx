...
export const Comment: FC<Props> = ({ comment, onSelectComment, onCancel, ...props }) => {
  const handleSelect = useCallback(
    (el: CommentType) => (event: MouseEvent) => {
  ...
  return (
    <div className={styles.comment} {...props}>
      <div
        className={classNames(styles.comment__container, {
      ...
      {!onCancel && (
        <div className={styles.comment__children}>
          {comment.children.map((el) => (
            <Comment comment={el} onSelectComment={onSelectComment} />
          ))}
        </div>
      )}
    </div>
